...
def build_report(summary: Mapping[str, Any], output_dir: str | Path) -> dict[str, str]:
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / "summary.json"
    json_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    csv_paths = []
    for section_key, title in (("main_methods", "main_methods"), ("diagnostic_methods", "diagnostic_methods")):
        rows = []
        columns = {"method"}
    ...
    def render_section(key: str, title: str) -> str:
        ...
        for method, payload in methods.items():
            ...
            rows.append(flat)
        columns = ["method"] + sorted({col for row in rows for col in row if col != "method"})
    ...
    html_path.write_text(
        ...
        "<style>body{font-family:Arial,sans-serif;margin:32px}table{border-collapse:collapse;font-size:12px}th,td{border:1px solid #ccc;padding:6px}th{background:#eee}</style>"
    ...
    outputs = {"json": str(json_path), "html": str(html_path), "csv": csv_paths}
    try:
        from openpyxl import Workbook
        workbook = Workbook()
        workbook.remove(workbook.active)
        for section_key, sheet_name in (("main_methods", "Main Methods"), ("diagnostic_methods", "Oracles")):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render adjustTM benchmark JSON summary to HTML and CSV")
...
def main() -> None:
    ...
    print(json.dumps(build_report(summary, args.output_dir), indent=2))
