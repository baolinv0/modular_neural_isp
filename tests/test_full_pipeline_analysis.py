...
from full_pipeline_analysis import FullPipelineAnalyzer, compute_luminance_stats, to_jsonable
...
class FakePipeline:
    ...
    def __call__(self, raw, **kwargs):
        ...
        self.assert_tensor_equal(kwargs['lsrgb'], self.post_ae)
        return {
            'raw': self.raw,
            'denoised_raw': self.denoised,
            'lsrgb': self.post_ae,
...
class FullPipelineAnalyzerTests(unittest.TestCase):
    def test_run_uses_capture_then_tone_pass(self):
        ...
        self.assertEqual(result['report']['tone_parameters']['gtm'], [[1.0, 2.0, 3.0]])
